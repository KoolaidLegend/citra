...
static OGLVertexArray attributeless_vao;
static constexpr char fullscreenquad_vs[] = R"(
#version 330 core
...
void main() {
    vec2 tbo_coord = (gl_FragCoord.xy - vp_offset) * tbo_size / vp_size;
...
static void ConvertD24S8toABGR(GLuint src_tex, const MathUtil::Rectangle<u32>& src_rect,
    ...
    glBufferData(GL_PIXEL_PACK_BUFFER, src_rect.GetWidth() * src_rect.GetHeight() * 4, nullptr,
                 GL_STREAM_COPY);
    ...
    // PBO now contains RABG
    state.draw.shader_program = d24s8_abgr_converter.shader.handle;
    ...
    glTexBuffer(GL_TEXTURE_BUFFER, GL_RGBA8, pbo.handle);
    glUniform1i(d24s8_abgr_converter.u_tbo, 0);
    glUniform2f(d24s8_abgr_converter.u_tbo_size, static_cast<GLfloat>(src_rect.GetWidth()),
                static_cast<GLfloat>(src_rect.GetHeight()));
    glUniform2f(d24s8_abgr_converter.u_vp_offset, static_cast<GLfloat>(dst_rect.left),
                static_cast<GLfloat>(dst_rect.bottom));
    glUniform2f(d24s8_abgr_converter.u_vp_size, static_cast<GLfloat>(dst_rect.GetWidth()),
                static_cast<GLfloat>(dst_rect.GetHeight()));
...
RasterizerCacheOpenGL::RasterizerCacheOpenGL() {
    ...
    d24s8_abgr_converter.u_tbo_size =
        glGetUniformLocation(d24s8_abgr_converter.shader.handle, "tbo_size");
    d24s8_abgr_converter.u_vp_offset =
        glGetUniformLocation(d24s8_abgr_converter.shader.handle, "vp_offset");
    d24s8_abgr_converter.u_vp_size =
        glGetUniformLocation(d24s8_abgr_converter.shader.handle, "vp_size");
}

RasterizerCacheOpenGL::~RasterizerCacheOpenGL() {
    FlushAll();
    d24s8_abgr_converter.shader.Release();
