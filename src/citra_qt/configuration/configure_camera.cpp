...
ConfigureCamera::ConfigureCamera(QWidget* parent)
    ...
    //    connect(ui->camera_outer_right_drop_area, SIGNAL(changed(const QMimeData*)), this, SLOT
    //    (OuterRightCameraDrop(const QMimeData*))); connect(ui->camera_outer_left_drop_area,
    //    SIGNAL(changed(const QMimeData*)), this, SLOT (OuterLeftCameraDrop(const QMimeData*)));
    //    connect(ui->camera_inner_drop_area, SIGNAL(changed(const QMimeData*)), this, SLOT
    //    (InnerCameraDrop(const QMimeData*)));

    connect(ui->camera_outer_right_mode, SIGNAL(currentIndexChanged(int)), this,
            SLOT(OuterRightCameraModeChanged(int)));
    connect(ui->camera_outer_left_mode, SIGNAL(currentIndexChanged(int)), this,
            SLOT(OuterLeftCameraModeChanged(int)));
    connect(ui->camera_inner_mode, SIGNAL(currentIndexChanged(int)), this,
            SLOT(InnerCameraModeChanged(int)));
...
void ConfigureCamera::setConfiguration() {
    ...
    // outer right camera
    if (ui->camera_outer_right_mode->itemText(0).toStdString() ==
    ...
    ui->camera_outer_right_drop_area->SetImage(
    ...
    ui->camera_outer_left_drop_area->SetImage(
    ...
    ui->camera_inner_drop_area->SetImage(
